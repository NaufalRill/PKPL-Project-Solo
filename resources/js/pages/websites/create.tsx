...
// ----- Types from controller props -----
type Client = { id: string; name: string };
type Feature = { id: string; name: string };
type PageProps = { clients: Client[]; features: Feature[] };
...
export default function CreateWebsite() {
    ...
    const featureOptions: Feature[] = props.features ?? [];

    const breadcrumbs: BreadcrumbItem[] = useMemo(
        () => [
            { title: t('app.websites'), href: route('websites.index') },
            { title: t('app.create_website'), href: route('websites.create') },
        ],
        [t],
    );
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        faq_display_mode: 0,
        external_link_display_mode: 0,
        clients: [] as string[], // array of client IDs
        features: [] as string[], // array of feature IDs (slug: blog, form, faq, external-link)
    });
    ...
    const filteredClients = useMemo<Client[]>(() => {
        ...
        return q ? clientOptions.filter((c: Client) => c.name.toLowerCase().includes(q)) : clientOptions;
    ...
    const submitNewClient: FormEventHandler = (e) => {
        ...
        router.post(route('clients.store'), clientForm.data, {
            preserveScroll: true,
            onSuccess: () => {
                // Refresh daftar clients saja (controller create() harus mengirim ulang props `clients`)
    ...
    const selectedClientChips = useMemo<{ id: string; name: string }[]>(
        () =>
            data.clients
                .map((id) => clientOptions.find((c: Client) => c.id === id) ?? null)
    ...
    return (
        <MainLayout breadcrumbs={breadcrumbs} title={t('app.create_website')}>
            <Head title={t('app.create_website')} />
            ...
            <form onSubmit={submitWebsite} className="mt-8 space-y-10">
                ...
                <section className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
                        ...
                        <div className="grid gap-2">
                            <Label htmlFor="order_number">{t('app.order_number') ?? 'Nomor Order'}</Label>
                            <Input
                                id="order_number"
                        ...
                        <div className="grid gap-2">
                            ...
                            <Input id="deployed_at" type="date" value={data.deployed_at} onChange={(e) => setData('deployed_at', e.target.value)} />
                ...
                <div className="flex items-center gap-3">
                    ...
                    <Button type="submit" disabled={processing}>
                        {t('app.create_website') ?? 'Create Website'}
                    </Button>
                </div>
            </form>
